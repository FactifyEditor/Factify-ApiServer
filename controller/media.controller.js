...
const { uploadBufferAudio, uploadRSSXML, uploadRSSFeeds } = uploadHandler;
const { convertTextMp3 } = ttsHelper;
const { generateRSSFeed, generateRSSFeeds } = rssFeedHelper
...
const publishMedia = async(req, res) => {
    try {
        ...
        const rssFeed = generateRSSFeeds(allPublishedFeed);
        let fedUrl = await uploadRSSFeeds(rssFeed);
        console.log(rssFeed);
